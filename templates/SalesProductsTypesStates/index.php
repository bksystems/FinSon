 ...
 * @var \App\Model\Entity\SalesProductsTypesState[]|\Cake\Collection\CollectionInterface $salesProductsTypesStates
 */
?>
...
<div class="row">
    ...
    <thead>
                <tr>
                    ...
                    <th><?= $this->Paginator->sort('type_state') ?></th>
            ...
            <tbody>
                <?php foreach ($salesProductsTypesStates as $salesProductsTypesState): ?>
                <tr>
                    <td><?= h($salesProductsTypesState->uuid) ?></td>
                    <td><?= h($salesProductsTypesState->type_state) ?></td>
                    <td><?= h($salesProductsTypesState->enable) ?></td>
                    <td><?= h($salesProductsTypesState->created) ?></td>
                    <td><?= h($salesProductsTypesState->modified) ?></td>
                    <td class="actions">
                        <?= $this->Html->link(__('Detalles'), ['action' => 'view', $salesProductsTypesState->uuid], ['class' => 'text-info']) ?>
                        <?= $this->Html->link(__('Editar'), ['action' => 'edit', $salesProductsTypesState->uuid], ['class' => 'text-warning']) ?>
                        <?= $this->Form->postLink(__('Eliminar'), ['action' => 'delete', $salesProductsTypesState->uuid], ['class' => 'text-danger', 'confirm' => __('Esta seguro de eliminar el registro # {0}?', $salesProductsTypesState->uuid)]) ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
